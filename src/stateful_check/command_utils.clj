...
(defn check-postcondition
  ...
  key describing the failure. Failing Clojure Test report events are
  captured and added to the :events key of the map. "
      ...
      (binding [report (fn [event] (swap! events conj event))]
        (let [postcondition-result (postcondition prev-state next-state args result)
          ...
          (cond
            ...
            ;; postcondition.
            (seq failure-events) {:message "Postcondition assertion failures."
